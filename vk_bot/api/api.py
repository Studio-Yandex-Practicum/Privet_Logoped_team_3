...
class Roles:
    ...
    @staticmethod
    async def _post_id_with_role(user_id, role):
        response = await async_http_post(
            bot_env.url_api + 'profile/uid/',
            data=Roles.fill_user_data(user_id, role)
        )
        print(response)
        log.info('Response: %s', response)
        return response
    ...
    @staticmethod
    async def send_role_for_id(user_id, role):
        user_id = str(user_id)
        response = await Roles._get_id(user_id)
        print(response)
        if response['status'] == HTTPStatus.NOT_FOUND:
            log.info('Not found user, create new')
            await Roles._post_id_with_role(user_id, role)
            ...
            if not Roles.check_user_role(current_user.role, role):
                ...
                response = await Roles._modify_user(user_id, role)

        log.info('Response: %s', response)
        return True

    @staticmethod
    async def _modify_user(user_id, role):
    ...
    @staticmethod
    def fill_user_data(user_id, role):
        return {
            'user_id': user_id,
            'username': user_id,
            'platform': 'vk',
